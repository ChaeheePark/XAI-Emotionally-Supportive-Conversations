...
class Model(BaseModel, BlenderbotSmallForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs
        ...
        if validation:
            # self.toker.vocab_size = 54944
            lm_logits = lm_logits[..., :self.toker.vocab_size+20].contiguous()
    ...
    def predict_strategy_and_emotion(self, logits, encoded_info):
        assert not self.training

        emotion_id = encoded_info.get('emotion_id', None)
        e_logits = logits[:, 0, -20:-8]

        if emotion_id is not None:
            pred = emotion_id
        else:
            if SAMPLE:
                filtered_logits = top_k_top_p_filtering(e_logits / TEMPERATURE, top_p=0.9)
                pred = torch.multinomial(F.softmax(filtered_logits, dim=-1), num_samples=1).squeeze(-1)
            else:
                pred = torch.argmax(e_logits, dim=-1)

        pred_top1 = torch.topk(e_logits, k=1, dim=-1)[1]
        pred_top3 = torch.topk(e_logits, k=3, dim=-1)[1]

        encoded_info.update({
            'pred_emotion_id': pred,
            'pred_emotion_id_top1': pred_top1,
            'pred_emotion_id_top3': pred_top3,
            'pred_emotion_id_dist': F.softmax(e_logits, dim=-1)
        })
        ...
        if strat_id is not None:
            ...
            if SAMPLE:
                filtered_logits = top_k_top_p_filtering(s_logits / TEMPERATURE, top_p=0.9)
                pred = torch.multinomial(F.softmax(filtered_logits, dim=-1), num_samples=1).squeeze(-1)
            else:
                pred = torch.argmax(s_logits, dim=-1)

        pred_top1 = torch.topk(s_logits, k=1, dim=-1)[1]
        pred_top3 = torch.topk(s_logits, k=3, dim=-1)[1]

        encoded_info.update({
            'pred_strat_id': pred,
            'pred_strat_id_top1': pred_top1,
            'pred_strat_id_top3': pred_top3,
            'pred_strat_id_dist': F.softmax(s_logits, dim=-1)
        })
    ...
    def generate(
        ...
        **kwargs
        ...
        self.predict_strategy_and_emotion(lm_logits, encoded_info)

        decoder_input_ids = torch.cat([decoder_input_ids,
                                       encoded_info['pred_emotion_id'][..., None] + len(self.toker) - 20,
                                       encoded_info['pred_strat_id'][..., None] + len(self.toker) - 8], dim=-1)  # -8 => 54956~54963
