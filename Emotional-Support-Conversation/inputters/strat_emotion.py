...
class InputFeatures(object):
    def __init__(
        ...
        self.decoder_input_length = len(decoder_input_ids)  # 42
        self.labels = labels

        self.input_len = self.input_length + self.decoder_input_length  # 160+42=202


def featurize(
        bos, eos,
        context, max_input_length,
        response, max_decoder_input_length, emotion_id, strat_id,
):
    ...
    # (max_decoder_input_length+2) = (42)
    labels = ([emotion_id] + [strat_id] + response + [eos])[:max_decoder_input_length + 2]
    # (max_decoder_input_length+2) = (42)
...
def convert_data_to_inputs(data, toker: PreTrainedTokenizer, **kwargs):
    ...
    for i in range(len(dialog)):
        ...
        if dialog[i]['speaker'] == 'sys':
            ...
            strat_id = process('[' + dialog[i]['strategy'] + ']')  # 54956~54963
...
def convert_inputs_to_features(inputs, toker, **kwargs):
    ...
    for i in range(len(inputs)):
        ...
        feat = featurize(
            ...
            ipt['response'], max_decoder_input_length, ipt['emotion_id'], ipt['strat_id'],
...
class FeatureDataset(Dataset):
    ...
    def collate(features: List[InputFeatures], toker: PreTrainedTokenizer, infer=False):
        ...
        if not infer:
            # (batch_size, max_decoder_input_length+2) = (16, 42)
        ...
        # (batch_size) = (16) 0~11
        emotion_id = torch.tensor([f.labels[0] for f in features], dtype=torch.long) - len(toker) + 20
        # (batch_size) = (16) +20 => 12~19 / +8 => 0~7
        strat_id = torch.tensor([f.labels[1] for f in features], dtype=torch.long) - len(toker) + 8
